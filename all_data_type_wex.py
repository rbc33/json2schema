...
def print_and_save_schema(json_file_path):
    ...
    output_file_path = os.path.splitext(json_file_path)[0] + '_schema_wex.txt'
            ...
            if isinstance(value, dict):
                ...
                if isinstance(value[0], dict):
                    ...
                    for index, item in enumerate(value):
                        output_file.write(f"{indent}  [{index}]: {type(item).__name__}      ex: {item}\n")
                        print(f"{indent}  [{index}]: {type(item).__name__}      ex: {item}")
    else:
        output_file.write(f"{indent}{type(data).__name__}       ex:{data}\n")
        print(f"{indent}{type(data).__name__}       ex:{data}")

folder_path = 'detequipos'
